...
class EventAdapter(
    ...
    inner class EventViewHolder(
        ...
        fun bind(event: Event) {
            ...
            val now = Instant.now()
            val upcoming = !DateFormatter.isInPast(event.dateTime, now)

            binding.tvDay.text = DateFormatter.dayOfMonth(event.dateTime)
            binding.tvMonth.text = DateFormatter.monthShort(event.dateTime)
            binding.tvTitle.text = event.title

            binding.tvDescription.text = event.description
            binding.tvDescription.isVisible = event.description.isNotBlank()

            binding.tvDateTime.text = DateFormatter.format(event.dateTime)
